...
// ═══════════════════════════════════════════════════════
// CARRIER COVERAGE DATA (from research-verified v2 database)
// ═══════════════════════════════════════════════════════
...
const MANDATE_STATES = { CT: 10, DE: 8, MD: 10, NJ: 8, VT: 8, WV: 8 };
...
function resolveCarrier(insurer, planType, state, tricarePlan) {
...
function getCarrierData(carrierKey, insurer) {
...
function calculateProbability(answers, boostAnswers = {}) {
  const { state, insurer, planType, employerSize, diagnoses, bmiRange } = answers;
  const { priorMeds = [], lifestyleProgram, labWork = [], weightDuration, additionalConditions = [], tricarePlan } = boostAnswers;
  const hasDx = (id) => diagnoses.includes(id) || additionalConditions.includes(id);
  ...
  ["wegovy", "zepbound", "mounjaro", "ozempic"].forEach((med) => {
    // Determine best indication key
    let indication, indicationLabel, key;
    if (med === "wegovy") {
    ...
    const data = carrier[key] || GENERIC_CARRIER[key] || { prob: [10, 25], pa: true, notes: "" };
    let low = data.prob[0], high = data.prob[1];

    // If exclusion (0%), don't modify
    if (low === 0 && high <= 5) {
    ...
    // Mandate state boost (not for large/self-funded employers)
    ...
    // Additional conditions from boost Q5
    ...
    if (hasDx("snoring")) dxBoost += 5; // potential undiagnosed OSA
    low += dxBoost; high += dxBoost;

    // ── BOOST QUESTION MODIFIERS ──
    const boosts = [];

    // Q1: Prior medications
    ...
    // Q3: Lab work (can also shift indications)
    ...
    // Indication overrides for non-indicated meds
...
function buildRoadmap(med, indication, data, answers, boostAnswers, hasStepTherapy, boosts) {
  const items = [];
  const { priorMeds = [], lifestyleProgram, tricarePlan } = boostAnswers;
  ...
  // What could improve odds (amber) — only show if they haven't done it
  if (!hasStepTherapy && !priorMeds.includes("none")) {
...
function ProbabilityBar({ prob, medName, indication, note, boosts, roadmap, animate }) {
  const color = prob >= 60 ? B.green : prob >= 30 ? B.amber : B.red;
  const bgColor = prob >= 60 ? B.greenLight : prob >= 30 ? B.amberLight : B.redLight;
...
function RoadmapSection({ items }) {
  ...
  const Section = ({ title, color, bgColor, itemList }) => itemList.length === 0 ? null : (
...
export default function CoverageQuizV2() {
  // Phase: "landing" → "quiz" → "initial_results" → "boost" → "email" → "results"
  const [phase, setPhase] = useState("landing");
  const [quizStep, setQuizStep] = useState(0);
  const [boostStep, setBoostStep] = useState(0);
  const [answers, setAnswers] = useState({ insurer: "", planType: "", employerSize: "medium_500_4999", state: "", diagnoses: [], bmiRange: "" });
  const [boostAnswers, setBoostAnswers] = useState({ priorMeds: [], lifestyleProgram: "", labWork: [], weightDuration: "", additionalConditions: [], tricarePlan: "" });
  ...
  const quizSteps = [
    ...
    { id: "employerSize", question: "How big is your employer?", subtitle: "Larger employers are more likely to cover GLP-1 medications", type: "single", options: EMPLOYER_SIZES, showIf: () => answers.planType === "employer" },
  ...
  // Recalculate results whenever boost answers change
  useEffect(() => {
  ...
  const setBoost = (key, value) => setBoostAnswers(prev => ({ ...prev, [key]: value }));

  const advanceQuiz = () => {
    if (quizStep < quizSteps.length - 1) setQuizStep(quizStep + 1);
    else {
      // Calculate initial results and show them
  ...
  const handleMultiToggle = (key, id, isBoost = false) => {
    const setter = isBoost ? setBoost : setAnswer;
    const current = isBoost ? boostAnswers[key] : answers[key];
    if (id === "none") { setter(key, ["none"]); return; }
    const filtered = current.filter(x => x !== "none");
    setter(key, filtered.includes(id) ? filtered.filter(x => x !== id) : [...filtered, id]);
  };
    ...
    return (
      <QuizContainer progress={quizStep} total={quizSteps.length} progressLabel="Step 1 of 2 — Basic Info">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ maxHeight: 320, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
              {INSURERS.filter(ins => !searchQ || ins.toLowerCase().includes(searchQ.toLowerCase())).map(ins => (
                <Pill key={ins} selected={answers.insurer === ins} onClick={() => { setAnswer("insurer", ins); setSearchQ(""); setTimeout(advanceQuiz, 200); }}>{ins}</Pill>
              ))}
            </div>
          </div>
        )}
        ...
        {step.type === "single" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {step.options.map(opt => (
              <Pill key={opt.id} selected={answers[step.id] === opt.id} onClick={() => { setAnswer(step.id, opt.id); setTimeout(advanceQuiz, 200); }}>{opt.label}</Pill>
            ))}
          </div>
        )}
        ...
        {step.type === "multi" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {step.options.map(opt => (
              <Pill key={opt.id} selected={answers[step.id].includes(opt.id)} onClick={() => handleMultiToggle(step.id, opt.id)}>{opt.label}</Pill>
            ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ maxHeight: 320, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
              {STATES_LIST.filter(s => !searchQ || STATE_NAMES[s].toLowerCase().includes(searchQ.toLowerCase()) || s.toLowerCase().includes(searchQ.toLowerCase()))
                .map(s => (
                  <Pill key={s} selected={answers.state === s} onClick={() => { setAnswer("state", s); setSearchQ(""); setTimeout(advanceQuiz, 200); }}>{STATE_NAMES[s]}</Pill>
                ))}
            </div>
          </div>
        )}
  ...
  // Phase 2: Boost questions (with live probability bars)
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: B.light, display: "flex", justifyContent: "center", padding: "40px 20px" }}>
        <div style={{ maxWidth: 520, width: "100%" }}>
          ...
          {/* Live mini probability bars */}
          {results && (
            <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
              {["wegovy", "zepbound", "mounjaro", "ozempic"].map(med => {
          ...
          {step.type === "single" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {step.options.map(opt => (
                <Pill key={opt.id} selected={boostAnswers[step.id] === opt.id} onClick={() => { setBoost(step.id, opt.id); setTimeout(advanceBoost, 400); }}>{opt.label}</Pill>
              ))}
            </div>
          )}
          ...
          {step.type === "multi" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {step.options.map(opt => (
                <Pill key={opt.id} selected={boostAnswers[step.id].includes(opt.id)} onClick={() => handleMultiToggle(step.id, opt.id, true)}>{opt.label}</Pill>
              ))}
              {boostAnswers[step.id].length > 0 && (
        ...
        <div style={{ maxWidth: 440, width: "100%", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <input value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First name"
              style={{ padding: "14px 18px", borderRadius: 50, border: `2px solid ${B.mid}`, fontFamily: font, fontSize: 14, outline: "none" }} />
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" type="email"
              style={{ padding: "14px 18px", borderRadius: 50, border: `2px solid ${B.mid}`, fontFamily: font, fontSize: 14, outline: "none" }} />
            <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone (optional)" type="tel"
              style={{ padding: "14px 18px", borderRadius: 50, border: `2px solid ${B.mid}`, fontFamily: font, fontSize: 14, outline: "none" }} />
            <button onClick={() => { setResults(calculateProbability(answers, boostAnswers)); setPhase("results"); }}
  ...
  if (phase === "results" && results) {
    ...
    const bestMed = r.medications[r.bestMedication];

    // Adaptive CTA
    let ctaTitle, ctaBody, ctaButton, ctaPrice, ctaRoute;
    if (bestProb >= 65) {
      ...
      ctaButton = "Confirm My Coverage";
      ctaPrice = "$25 one-time eligibility check";
      ctaRoute = "/checkout?sku=INS-ELIG";
    } else if (bestProb >= 30) {
      ...
      ctaPrice = "$25 eligibility check — includes strategy consultation";
      ctaRoute = "/checkout?sku=INS-ELIG";
    } else {
      ...
      ctaPrice = "Starting at $139/mo";
      ctaRoute = "/quiz/weight-loss";
    }
      ...
      <div style={{ minHeight: "100vh", backgroundColor: B.light, display: "flex", justifyContent: "center", padding: "40px 20px" }}>
        <div style={{ maxWidth: 520, width: "100%" }}>
          ...
          {Object.entries(r.medications)
            ...
            .map(([med, data]) => (
              <div key={med}>
                <ProbabilityBar
                  prob={data.prob}
                  medName={med.charAt(0).toUpperCase() + med.slice(1) + (med === "wegovy" ? "®" : med === "zepbound" ? "®" : med === "mounjaro" ? "®" : "®")}
          ...
          <div style={{
            ...
            <div style={{ fontFamily: font, fontSize: 12, color: B.gray, marginTop: 8 }}>{ctaPrice}</div>
          </div>

          {/* Disclaimer */}
          <p style={{ fontFamily: font, fontSize: 10, color: B.gray, marginTop: 24, lineHeight: 1.5, textAlign: "center" }}>
            These results are estimates based on publicly available insurance formulary data. They are not guarantees of coverage.
