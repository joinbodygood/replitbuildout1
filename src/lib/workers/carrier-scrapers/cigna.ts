...
const SEARCH_URL = "https://www.cigna.com/individuals-families/member-resources/prescription-medications";
...
class CignaScraper extends CarrierScraper {
  carrierKey = "cigna";
  state = "_default";
  ...
  async scrape(): Promise<ScrapedRow[]> {
    ...
    for (const q of QUERIES) {
      ...
      await this.page.fill('input[aria-label*="search" i], input[name*="drug" i]', q.term);
      await this.page.press('input[aria-label*="search" i], input[name*="drug" i]', "Enter");
      await this.page.waitForLoadState("networkidle");

      const result = await this.page.locator('.drug-result, [data-cy="drug-result"]').first().count();
      if (result === 0) {
      ...
      const tierText = await this.page.locator('.drug-tier').first().textContent().catch(() => null);
      const tier = Number((tierText ?? "").replace(/\D/g, "")) || null;
      const paText = await this.page.locator('.pa-required').first().textContent().catch(() => "");
...
export async function run() {
  return runWorker("carrier-cigna", async () => {
    const s = new CignaScraper();
    await s.start();
    try {
...
if (require.main === module) {
  run().then(r => console.log("cigna done:", r)).finally(() => db.$disconnect());
}
