  ...
  async function search(q: string) {
    try {
      ...
      if (isZip) url += `&postal_code=${encodeURIComponent(q.substring(0, 5))}*`;
        ...
        url += `&organization_name=${encodeURIComponent(q)}*`;
        if (zip) url += `&postal_code=${encodeURIComponent(zip)}*`;
      }
      setLoading(true);
      ...
      setResults(data.results.map((r: Record<string, unknown>) => {
        ...
        return {
          name: basic?.organization_name || "Unknown",
          address: [addr.address_1, addr.city, addr.state, addr.postal_code?.substring(0, 5)].filter(Boolean).join(", "),
    ...
    <div ref={wrapRef} className="relative mt-2">
      <div className="grid grid-cols-3 gap-2 mb-2">
        <div className="col-span-2">
          <TextInput value={query} onChange={handleInput} placeholder="Search by pharmacy name..." />
        </div>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 z-10 bg-white border border-border rounded-b-lg shadow-lg max-h-52 overflow-y-auto">
          {results.length === 0 ? (
            <p className="text-xs text-body-muted px-3 py-3">No pharmacies found.</p>
          ) : results.map((p, i) => (
            <button
              key={i}
              type="button"
